...
#' convert_two_Quartiles_value
#'
#' 函数作用：此函数是将数据框中的多列批量转化为二分位数区间，分组列按照数值区间表示，例如将Cmax按照二分卫数区间分组
#' @param df 需要转换的数据框
...
#' )
#' #对指定列进行四分位数转换
#' df <- convert_two_Quartiles_value(df, column_names=c("value1", "value2"))
# 定义函数
convert_two_Quartiles_value <- function(df, column_names, suffix = "_quartiles",label_format ="Q_numeric")
{
  library(dplyr)
  if (2>1){
    A <- .convert_two_Quartiles_value(df, column_names, suffix = "_quartiles",label_format)
  }
  return(A)
}

.convert_two_Quartiles_value<- function(df, column_names, suffix = "_quartiles", label_format) {
  # 检查所有列名是否在数据框中
