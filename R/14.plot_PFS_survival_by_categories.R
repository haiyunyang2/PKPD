...
#' plot_PFS_survival_by_categories
#'
#' 函数作用：此函数是可以批量绘制PFS的KM曲线
#' @param data 原始数据
#' @param time_col 时间列
#' @param  event_col  删失列
...
#' #示例代码
#' plot_PFS_survival_by_categories(data,time_col="time",event_col="status",output_dir="",legend_titles=c("A","B"))
# 定义函数
plot_PFS_survival_by_categories <- function(data, time_col, event_col, categorical_vars, output_dir, legend_titles)
{
  library(dplyr)
  if (2>1){
    A <- .plot_PFS_survival_by_categories(data, time_col, event_col, categorical_vars, output_dir, legend_titles)
  }
  return(A)
}
